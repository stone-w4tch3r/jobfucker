---
name: jobfucker-creating-prompts
description: >-
  Создание и итерация промптов jobfucker: скоринг-промпт и личный apply-промпт (сопроводительное).
  Используй при создании/калибровке скоринг-промпта, разборе завышенных/заниженных оценок,
  поиске примеров/анти-примеров, а также при создании или правке сопроводительного письма
  (подпись, контакты, тон).
---
...
## Apply-промпт (сопроводительное)

Отдельный промпт: `apply_prompt_file` XOR inline `apply_prompt`. Контракт тот же, что у скоринга — ровно `{{ resume_formatted }}` + `{{ vacancy_formatted }}`, `StrictUndefined`; но на выходе **plain text** (никакого JSON), принимается дословно.

**Личный** промпт: подпись, имя, контакты, тон. «Правильного» готового нет. Пример `apply-java-fullstack.xml.j2` (в комплекте со скиллом, или в `docs/examples/`) — базовый каркас, не эталон.

**Спроси пользователя, ничего не выдумывай:**
- имя/подпись и как представляться;
- контакты в письме (Telegram, портфолио, PDF резюме) — включать или нет, какие;
- тон (тёплый / нейтральный / формальный) и длину;
- стиль приветствия;
- когда отходить от шаблона (просят ключевые слова, ответить на вопросы, тестовое);
- язык письма (обычно язык вакансии);
- насколько сильно персонализировать под вакансию/компанию.

**Структура** (как в примере): `<instructions>` — правила правки шаблона; `<template>` — готовый текст с контактами; инжекция `{{ resume_formatted }}` / `{{ vacancy_formatted }}`; `<final_task_summary>`.

**Проверка:** рендерится ровно с двумя переменными; текст письма непустой (пустое письмо не отправить на hh-тестовые вакансии); пайплайн сохранён через `init`/`update`.
...
- Apply-промпт: личные данные (имя, контакты, подпись, тон) пришли от пользователя; рендерится с двумя переменными; выход — plain text.
...
- Выдуманные контакты, подпись или тон в apply-промпте.
