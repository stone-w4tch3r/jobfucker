...
description: >-
  Верхнеуровневый операционный цикл jobfucker и роутер по скиллам: fetch → score → ревью и итерация промпта → triage requires_attention → generate → apply, плюс batch-флаги, откат скоринга, hh-тесты, status и лимиты.
...
Сетевые/платные шаги (`fetch`, `score`, `generate`, `apply`) — только по явному одобрению пользователя.
